...
双人音频 V4 使用证据、主张表和携带真实前文的章节生成。产品任务最多一次额外 MAIN 审校和一次共享技术恢复；大时长所需的正常分批不计作重试。严格脚本评测路径不经共享名额控制，其恢复是各自独立的单次有界机制：每个 Act 一次连接重试、整集一次空响应重试、续写／时长扩写／时长压缩三者居一。纯提问省略空引用数组、明确复合引用如 `C1|E1` 可本地恢复，未知编号不会挂到无关资料。检查相邻问答、章节衔接和完整结尾，不能把同属一个主题当作已经回答问题。首轮声称已有前文属于明确断裂，审校笼统通过不能覆盖该问题；同次审校最多返回连续六轮局部替换，未经独立二次审校会注明。无法恢复时只保留连续短版或草稿，不跨缺失章节拼接。时长、角色比例、问句比例和措辞风格是软提示，不据此重写整集。
...
严格脚本评测路径没有 delivery 包装：默认（conservative）路径在生成前固定预留 6000 tokens 作为整集终审准入额度，非终审 MAIN 调用一旦按校准准入估算侵入预留即被拒绝（报“已为最终整集审校保留预算”）；有界恢复调用在发起前先对称扩展任务上限（绝对上限 45,000 不变），保证终审始终可达。任务预算准入使用校准估算（提示约按 UTF-8 字节数 /4 估计，外加最小输出额度），按 Provider 计量值结算；失败请求退还预留、不消耗 token 预算，但仍计入调用次数上限。
...
- **保留改进**：iter-00 预算记账修复与 plan 视图精简、iter-01 深槽硬下限、iter-02 问句下限与主持人均衡；第二轮（iter-13–18）recovery 硬化六项（恢复预算授予时机、续写区间重述与解析失败可诊断、grant-before-call 对称化、续写 claim 绑定旋转、双层嵌套 turns 解包及其放宽）与 iter-18 open_hook 实质收尾；第三轮（iter-30–40）在双指标（bitcoin margin + strange 可评率）、K=3 复现、锚点漂移监测的方差削减协议下测试 10 个杠杆，零保留。
...
### 5–6 分钟短集（2026-09-19 修复）

此前 5 分钟模式在新旧代码上均确定失败（旧：act 无内容；新：压缩计划为空——18 轮密度下草稿 ~1.5× 超长且几乎无可压缩的事实轮）。现 ≤6 分钟短集改用 2.0 轮/分钟（5 分钟 = 10 轮）；整集轮数 ≤20 的短 episode 放宽压缩参数（中文 floor 28/margin 18，英文 14/9）。压缩容量不足目标削减量时不再放弃压缩，任何时长的 episode 都以全部候选继续，由 0.85–1.20 时长门禁裁决。实测 5 分钟通过率约 1/3，时长全部达标；剩余失败集中在 host_a_ratio / 问句比例 / 整集审校等边界门禁，与长 episode 的已知 drafting 方差同族。12 轮/14 轮密度实测更差（0/4），不要回调。
...
自 v0.4.8 起引擎14不复用旧脚本检查点，已有完成产物继续读取。生产 Provider 配置与上下文策略资格保持原值；不切换简化写作协议。此前短章节对照两套协议都未达到整集资格，本次技术验证不能替代三参考质量对标或人工听感验收。

### 高推理生成与轻量复测

Podcast engine 14 将超出目标章数的提纲按顺序归并，并保留各章的问题、机制、条件和引用归属。旧 engine checkpoint 不复用，已有成品仍可阅读。单章 `opening/body/closing` 可本地转换为章节映射，不为确定的格式差异重写正文。

`high` / `xhigh` / `max` 配置保持原推理强度：结构化首轮总输出额度目标至少 8192 tokens，手动输出上限与安全上下文仍生效；任务内复用同阶段成功额度。额外推理空间不扩大笔记或审查数量。conservative Podcast 高推理任务使用 45000 tokens 硬上限，实际消耗按调用计量；扩写必须为最终审核保留输入和输出空间。每任务最多一次技术恢复、一次审核，不能因为质量评级不够高而循环重试。

长书预读为最终生成、审核以及尚未使用的技术恢复预留调用；恢复发生后释放对应的预留名额。预读计划四批时，一次恢复不应挤掉第四批。真实预算不足、取消和未覆盖区域必须如实报告。

轻量对照可固定同一 Bitcoin 索引的中文 5 分钟脚本和同一 GEB 索引的中文摘要，分别使用 k3-256k（256000 窗口）及 k3（1048576 窗口），最大输出配置 100000、temperature=1、reasoning_effort=high；Podcast 为 conservative、摘要为 balanced。冻结源码、配置与参考；保留本地逐调用输出额度、推理消耗、阶段覆盖和停止原因。5 分钟脚本以 3.5–6.5 分钟作为观察目标，时长不单独触发失败；人工检查开场问题是否得到有原文支持的回答。自动审核待核实项与执行失败分别报告，不把预读更多材料或一次成功当成 1M 极限验证或生产资格通过。

高推理结构化请求的超时按估算输入与请求输出同步计算（最长 600 秒），避免仅放大输出却仍被 180 秒固定读取超时截断。章节替换会携带原短章的解释与引用要求；原文放不下时先在本地让出部分输出空间，仍不足则保留原短章。替换结果若完全偏离原短章依据，保留短章并标记降级；只遗漏部分引用则保留扩写并提示覆盖待核对，不触发新的质量重试。这项检查不证明语义或事实全部通过。

兼容服务返回的完整多轮正文（单章最多 32 轮）；多余结束括号可本地解析，但不能拼接损坏内容或第二个 JSON 对象。摘要仅兼容单层 points 嵌套与 text→claim 别名，不从诊断文本恢复主张。回放验证必须注明复用的历史调用；改动后的口播不能沿用旧稿审核结论，未追加审核时保持未验证状态。


### v0.4.8 发布验证

发布前以最终候选源码及固定 Bitcoin 样本隔离运行一次中文五分钟完整 Podcast，覆盖真实 MAIN、脚本落库、TTS 与 ASR。MAIN 复制目标部署配置，仅显式设置 `reasoning_effort=high`，不自动附加 `thinking`；AUDIO 复制实际目标配置及能力，不能把评测脚本内置的自动选择、音色或设备当作生产配置。现有上下文对照脚本的 `--audio` 使用固定 AUDIO 配置，精确部署对照需由本地启动器注入目标快照，并记录实际执行模型和设备。

单场最多 30 分钟，一次终审、一次共享技术恢复，不因质量评分或时长偏差循环生成。脚本完整、音频可播放、ASR 完成、状态真实是技术验收条件；明确对话断裂、损坏音频或执行失败阻止上线。一般事实疑点、未完整核验与时长偏差正常展示，不等于质量认证。新生成结果与保存回复回放分别报告；仅回放不能称为完整 MAIN 实测。鉴权、额度或外部服务故障时停止调用，不用重试掩盖失败。

此前轻量复测中两种 k3 摘要均有六条中文要点，256K/1M 自动原文支持为 5/6 与 3/6；修复后的 Podcast 保存回复回放分别保留约 4.26/3.2 分钟脚本，最终稿未重新审核。该证据不覆盖音频、所有参数或 1M 极限输入；发布验证结果另记于 release，原始材料仅在本地保存。
