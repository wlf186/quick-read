...
**5–6 分钟短集（2026-09-19 修复）**：此前 5 分钟模式在新旧代码上均确定失败（旧：act 无内容；新：压缩计划为空——18 轮密度下草稿 ~1.5× 超长且几乎无可压缩的事实轮）。现短集改用 2.0 轮/分钟（5 分钟 = 10 轮）并为短 episode 放宽压缩参数（floor 28/margin 18），压缩候选不足时以全部候选继续、由 0.85–1.20 时长门禁裁决。实测 5 分钟通过率约 1/3，时长全部达标；剩余失败集中在 host_a_ratio / 问句比例 / 整集审校等边界门禁，与长 episode 的已知 drafting 方差同族。12 轮/14 轮密度实测更差（0/4），不要回调。
...
引擎14不复用旧脚本检查点，已有完成产物继续读取。生产 Provider 配置与上下文策略资格保持原值；不切换简化写作协议。此前短章节对照两套协议都未达到整集资格，本次技术验证不能替代三参考质量对标或人工听感验收。
