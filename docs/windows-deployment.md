...
部分兼容服务不公开窗口元数据。此时应用会明确提示并按 4096 tokens 安全运行；最大输出按有效窗口的 25% 推导、上限 4096。可根据服务官方规格人工覆盖两项值。生成过程中还会预留安全余量，并只在服务明确返回上下文溢出时自动缩小证据后重试。对默认开启思考的模型（如 GLM-5.x），隐藏推理与可见输出共享输出预算；输出因上限结束且推理占主导时，系统会把输出预算翻倍重试一次（人工/服务上报上限不突破，推导上限最多放宽到 16384），也可在 Provider 设置中把“思考模式”改为“请求关闭思考”。

连接检查不会保存配置；深度验证只发送内置的极短测试文本或图片，但云服务仍可能计费。真正使用远程（局域网或云端）MAIN/VLM/AUDIO 时，问题上下文、资料片段、页面图片或 Podcast 文本会发送到所配置的服务，请在上传敏感资料前确认服务条款和组织合规要求。
...
v0.4.8 更新需重建前端并重启后端，没有新增数据库迁移。升级前确认无排队或执行中的任务，备份 runtime/config.toml、runtime/data（含凭据加密密钥）、旧版本代码及 frontend/dist；运行中复制 SQLite 文件可能不一致，应停服备份或使用 SQLite backup API。脚本检查点引擎升为14，未完成任务重启后可能重做生成；已完成产物兼容。Provider 的窗口值、模型、角色选择和上下文资格不因部署改写，实验性简化 Podcast 协议不启用。升级后检查版本、静态资源、历史产物和配置；需回退时恢复旧代码与前端，勿直接用旧数据库覆盖升级后的新写入。


v0.4.8 无新增依赖，bootstrap 用于同步项目版本并重建前端。高推理预算保护仅在 MAIN 的 `config.reasoning_effort` 明确为 `high`、`xhigh` 或 `max` 时启用，服务默认推理强度不会被自动猜测。需要时先用目标配置隔离验证，再保留完整 config 对象添加该字段；不必同时更改 `thinking`。部署不会自动切换模型、窗口或上下文策略。回退本次配置改动时保留其他配置和升级后的运行数据。

v0.4.9 更新需重建前端并重启后端，无新增数据库迁移或依赖升级；脚本检查点引擎保持 14。AUDIO Provider 未配置或不可用时 Podcast 降级为仅交付双人对话脚本（script_only），产物抽屉支持打包下载（ZIP 含音频、极简 Markdown 脚本与完整 JSON）或单独下载任一部分；Studio 入口更名为“双人Podcast”。

v0.4.10 更新需重建前端并重启后端，无新增数据库迁移或依赖升级；bootstrap 会同步安装元数据。脚本检查点引擎升为 15（生成策略与短集修复所致），未完成的 Podcast 任务重启后可能整集重做生成；已完成产物保持兼容读取。部署不改写 Provider 窗口、模型、角色选择与上下文资格。Windows 命令与本节流程不变。
...
本次更新需要运行 bootstrap 重新构建前端并重启后端；只刷新浏览器不会加载新的后端逻辑。已有完成的产物和 Provider 配置保留，不会因升级自动重新生成。未完成 Podcast 的脚本缓存签名已变化，后续重新执行时可能重做脚本和关联音频，应预留模型计算时间。部分产物会显示实际数量/时长及验收偏差；任务完成不表示所有质量检查通过。
...
动态上下文更新需要重建前端并重启后端。Provider 中的两个窗口值会影响选材和分批。均衡策略下，摘要和问答累计预算上限为 max(300000, ceil(1.25 × 有效上下文窗口))，短资料按需求下调，其它功能仍最高 300000 tokens；1M 窗口不代表每项任务都会发送 1M 内容。配置预估为本地计算，显示预读批次、笔记及摘要容量；多资料摘要增加分资料要点，实际覆盖显示在聊天或产物中，旧产物没有该字段时正常展示。没有新增数据库迁移。

新旧策略对照参见 [上下文策略对照](podcast-evaluation.md#5-上下文策略对照)，PowerShell 使用 `.\.venv\Scripts\python.exe`，样本路径加引号；隔离实例的工具与模型链接仍要求开发者模式或创建链接权限。

先完成 bootstrap，准备实际存在的 PDF，并将以下服务地址替换为自己的 MAIN/AUDIO；该工具使用独立配置，不继承网页里现用的 Provider。

```powershell
.\.venv\Scripts\python.exe scripts/evaluate_generation.py --output runtime/evals/gemma-baseline --phase baseline --sample "C:\documents\sample.pdf" --main-url http://127.0.0.1:11434 --model gemma4:e4b --audio-url http://127.0.0.1:20810
```

MAIN 默认30720上下文/4096输出，可用 `--context-window`、`--max-output-tokens` 覆盖，AUDIO 需要 Qwen3 TTS/ASR 0.6B 与相应音色；会真实发送资料片段、口播文本及待验收音频。共享工具/模型目录使用符号链接，Windows 需启用开发者模式或具备创建链接权限。参数选择、续跑限制及输出文件见 [综合功能实测指南](podcast-evaluation.md#4-综合功能实测)。退出码 0 仍可能包含降级产物；ASR 与实际时长需分别查看，严格 Podcast/TTS 资格评测继续要求原门槛通过。


## Office 文档导入排查

当前原生支持 DOCX、PPTX、XLSX；旧 DOC/PPT/XLS 请先另存为对应的新格式。升级后运行项目 bootstrap 同步新增的 XLSX 解析依赖。XLSX 公式使用文件保存的结果，未保存计算缓存时会显示提示；可在 Excel 中重新计算并保存后再上传。

LibreOffice 预览转换的超时、启动失败或控制台编码异常不会清除已经提取的正文；界面会提示无法预览的视觉内容。转换输出按字节接收，避免 Windows 控制台编码与文件名造成解码失败。单次转换超时仍为 180 秒。

PDF 慢时检查资料元数据中的 `ingest_timings`：原生解析、预览转换、视觉服务和向量索引分别计时。首次模型加载与后续热运行分开比较。不会为了提速跳过扫描页，也不会提前将尚在处理的资料标成就绪。完整隔离验证流程见 [资料导入评测](document-import-evaluation.md)。
