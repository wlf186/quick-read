...
应用本身可以在没有生成式模型的情况下导入、索引和检索资料，并在模型不可用时返回相关原文摘录。完整问答、Studio 内容和播客生成需要配置 MAIN；图片理解需要 VLM；语音合成需要 TTS。
...
3. 需要识别图片时，以同样方式编辑 `Local Vision`（VLM），选择支持视觉的模型。

如果某个旧部署仍显示 `http://iollama:11434`，这是旧配置数据库中已经保存的地址；在设置页改为 `http://127.0.0.1:11434` 即可。

### 4.2 OpenAI-compatible 云端服务

在 Provider 设置中新增 MAIN，类型选择 `OpenAI-compatible`，填写服务地址、API Key 和模型。地址可以带常见的 `/v1`、`/api` 或 `/api/v1`，保存时会规范化。先“连接并读取模型”，无法读取清单时可手填模型，再执行“深度验证”。视觉模型应另建 VLM Provider。

连接检查不会保存配置；深度验证只发送内置的极短测试文本或图片，但云服务仍可能计费。真正使用云端 MAIN/VLM/TTS 时，问题上下文、资料片段或页面图片会发送到所配置的服务，请在上传敏感资料前确认服务条款和组织合规要求。

### 4.3 TTS 语音服务

可新增 `OpenAI TTS` Provider，填写兼容服务地址、Key、模型和音色。若要全本地播客语音，可部署同组织的 [audio-intel](https://github.com/wlf186/audio-intel) 服务：
...
返回 quick-read，在设置中编辑 `Sandevistan TTS`，地址填 `http://127.0.0.1:20810`，读取音色并深度验证。TTS 模型对磁盘、内存和 GPU 的要求明显高于主应用，安装前请阅读 [audio-intel Windows 手册](https://github.com/wlf186/audio-intel/blob/main/docs/WINDOWS.md)。
...
| 模型清单为空 | 服务可能不提供清单接口；手填模型后执行深度验证。Ollama 则先运行 `ollama pull <模型>`。 |
