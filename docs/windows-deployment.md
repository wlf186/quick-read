...
动态上下文更新需要重建前端并重启后端。Provider 中的两个窗口值会影响选材和分批，但长任务总预算仍最高 300000 tokens；1M 窗口不代表每项任务都会发送 1M 内容。配置预估为本地计算，实际覆盖显示在聊天或产物中，旧产物没有该字段时正常展示。没有新增数据库迁移。
...
MAIN 固定 30720 上下文，AUDIO 需要 Qwen3 TTS/ASR 0.6B 与相应音色；会真实发送资料片段、口播文本及待验收音频。共享工具/模型目录使用符号链接，Windows 需启用开发者模式或具备创建链接权限。参数选择、续跑限制及输出文件见 [综合功能实测指南](podcast-evaluation.md#4-综合功能实测)。退出码 0 仍可能包含降级产物；ASR 与实际时长需分别查看，严格 Podcast/TTS 资格评测继续要求原门槛通过。
