...
本次生成恢复更新需要运行 bootstrap 重新构建前端并重启后端；只刷新浏览器不会加载新的后端逻辑。已有完成的产物和 Provider 配置保留，不会因升级自动重新生成。未完成 Podcast 的脚本缓存签名已变化，后续重新执行时可能重做脚本和关联音频，应预留模型计算时间。部分产物会显示实际数量/时长及验收偏差；任务完成不表示所有质量检查通过。
