...
v0.4.6 更新需重建前端并重启后端，没有新增数据库迁移。升级前确认无排队或执行中的任务，备份 runtime/config.toml、runtime/data（含凭据加密密钥）、旧版本代码及 frontend/dist；运行中复制 SQLite 文件可能不一致，应停服备份或使用 SQLite backup API。脚本检查点引擎升为13，未完成任务重启后可能重做生成；已完成产物兼容。Provider 的窗口值、模型、角色选择和上下文资格不因部署改写，实验性简化 Podcast 协议不启用。升级后检查版本、静态资源、历史产物和配置；需回退时恢复旧代码与前端，勿直接用旧数据库覆盖升级后的新写入。
