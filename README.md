...
学习内容采用“知识蓝图 → 候选生成 → 证据与质量校验 → 去重”的流水线。系统综合模型参数量、有效上下文窗口和最大输出窗口自动选择 `full` 或 `lite` 档，也可在 Provider 配置中覆盖；`lite` 档不生成困难题，闪卡在 `lite` 档使用更多候选轮次以兼容小模型。题卡只有通过逐项引用、证据一致性和结构质量检查后才会保留，数量不足时会明确返回部分产物，不使用模板填充凑数。去重同时要求向量语义高度相似与字面内容重合（短中文文本在同主题下向量余弦普遍虚高，仅凭余弦会误杀不同卡片）；只产出重复候选的轮次不会触发提前停止，只有完全无法产出有效候选才提前结束。
