...
Podcast 按大纲逐章深化：完整短稿保留章节对应关系，深入版本成功后原位替换；失败保留该章短版，不重复附加整集概括。同一主张可用于不同机制与条件的解释。允许口播明确标识的解释性类比，仍核验机制与原文归属；事实疑点、章节回退和音频/时长结果分别报告。预算与一次恢复、一次终审上限不变。

Podcast 按完整原文证据组规划与写作，保留相邻片段中的必要条件；预读笔记仅辅助理解，不替换同段其他内容。无关注点时按资料和区域分配选材，短稿接收每章完整主张集合，优先解释机制与条件。证据组不足预算时整体略过而不截断条件；选材或引用覆盖不代表知识完整覆盖。现有预算、一次技术恢复和一次终审保持不变，质量不足仍保留可交付结果。
...
当前 v0.4.7 修复了模型错误标注 `Q` 时完整陈述被误删的问题：问句判断结合实际口播文本，保留原话、顺序和引用；真实未答问题及损坏数据仍受结构检查约束。资料覆盖按章节写作、技术恢复分别显示，阶段间可能重复，不应相加视为完整知识覆盖。脚本检查点引擎为13，旧成品兼容。此次发布改善稳定性与可观察性，不表示 Gemma 已达到三参考音频的质量标准；实验性简化协议和未获资格的上下文策略不会自动启用。

v0.4.7 页脚版本直接读取后端运行版本，状态未就绪时显示占位符，不再写死 BUILD 编号。
