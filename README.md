...
问答、摘要、闪卡和播客脚本会预留 20% 安全余量，并按输入与输出预算裁剪证据。若 Provider 仍报告上下文溢出，系统会以 100%、50%、25% 三档自动缩小后重试；网络连接或超时错误则在同一预算下重试一次，不消耗溢出降档。思考模型的隐藏推理与可见输出共享同一个输出预算：当输出因达到上限结束且推理占主导（或内容为空）时，系统在同一档位把输出预算翻倍重试一次——人工或服务上报的输出上限是硬顶，推导上限最多放宽到 16384。发生裁剪、输出受限或回退时，聊天或 Studio 产物会显示“已按模型窗口调整证据量”。
...
检索范围固定为当前勾选资料的修订快照；回答要求逐项使用 `[S1]` 引用，服务端只返回真实检索片段对应的引用元数据。Ollama 暂时性 5xx 故障会在同一 Provider、同一预算下最多重试一次；模型仍不可用时返回相关原文摘录，不使用未上传的外部知识。低输出预算的摘要分批生成要点，并保留截断响应中已完整且有引用的要点。点击 UI 引用可查看文件名、定位与原文证据。
...
学习内容采用“知识蓝图 → 候选生成 → 证据与质量校验 → 去重”的流水线。系统综合模型参数量、有效上下文窗口和最大输出窗口自动选择 `full` 或 `lite` 档，也可在 Provider 配置中覆盖；所有档位均尝试所选难度，批次按最大输出预算调整；闪卡在 `lite` 档使用更多候选轮次以兼容小模型。题卡只有通过逐项引用、证据一致性和结构质量检查后才会保留；字面重合不足的候选需通过语义证据审校，以支持跨语言生成。只要有有效题卡，数量不足时会返回部分产物，并通过 `quality_report` 与 `warnings` 说明数量、难度或审校完成情况；零有效内容仍报错，不使用模板填充凑数。去重同时要求向量语义高度相似与字面内容重合（短中文文本在同主题下向量余弦普遍虚高，仅凭余弦会误杀不同卡片）；只产出重复候选的轮次不会触发提前停止，只有完全无法产出有效候选才提前结束。
...
交互生成保留通过现有检查的有效部分，数量、风格、时长或审校不足时显示降级说明和原始指标，不使用重复内容凑数。事实审校定位的无依据轮次会移除；没有可保留的双人对话、首次媒体合成失败或任务取消时仍不会交付成品。已有可播放音频不会仅因时长或 ASR 未通过而丢弃；局部音频修复失败时保留修复前成品。

支持兼容批量协议时，多轮 TTS 可共用模型加载；MAIN 与 AUDIO 地址满足条件时，已完成段落的合成可与后续脚本生成重叠。Provider 中关闭 `podcast_sequence_tts` 会同时关闭批量与重叠。任务分别显示脚本、等待合成和 ASR 阶段。协议限制、缓存复用和有限恢复细节见 [评测指南的交互生成说明](docs/podcast-evaluation.md#交互生成与严格评测)。
...
均衡策略已实现，目前仍处于质量验证阶段，默认保留保守策略。显式设置 MAIN Provider 的 `config.context_strategy="balanced"` 才启用下述动态选材；`conservative` 可回退。容量预估不代表已通过质量资格，生产配置不会自动更改。

MAIN 的上下文窗口与最大输出共同决定选材和生成批次，不按模型名称划分固定容量。短资料直接综合；多资料先按 token 预算分配选材份额，再将剩余预算用于补充；长资料按章节、EPUB 区段或连续页区间分布选材，必要时先分批提炼带原文定位的要点。摘要预读只用于选择片段，最终综合重新读取完整原文，并在独立核验中检查陈述者、虚构示例、条件及结论；未核验的要点不会作为合格综合结果保留。摘要、题卡与 Podcast 会继续使用原文证据核验，问答最多追加一次相邻证据检索。选材更多不等于已读完全部资料，也不保证重要信息全部覆盖。
...
保留 20% 上下文安全余量，单阶段原文预算最高 64000 tokens，问题检索最高 32000；长任务 MAIN 总预算最高 300000 tokens，至少预留 25% 用于最终综合、审校和恢复。提炼最多 4 批，输出预算用于控制每批规模，不会强制填满窗口或输出长度。原文选材另受均衡任务预算限制，因此 1M 上下文不会自动发送 1M 内容。失败重试也计入预算，预算不足时保留有效部分并报告偏差。

聊天和产物的“本次资料覆盖”区分候选、选材、实际送入成功调用、部分送入和最终引用。章节／页区间涉及率不是重要信息召回率；未列入选材的内容不会因任务完成而算作已读。Quiz 作答前仅提供聚合统计。旧产物没有覆盖字段时保持原有展示。
