...
当前 v0.4.6 修复了模型错误标注 `Q` 时完整陈述被误删的问题：问句判断结合实际口播文本，保留原话、顺序和引用；真实未答问题及损坏数据仍受结构检查约束。资料覆盖按章节写作、技术恢复分别显示，阶段间可能重复，不应相加视为完整知识覆盖。脚本检查点引擎为13，旧成品兼容。此次发布改善稳定性与可观察性，不表示 Gemma 已达到三参考音频的质量标准；实验性简化协议和未获资格的上下文策略不会自动启用。
