...
function ContextCapacity({providerId,model,config,limits,notebookId,sourceIds}:{providerId?:string;model:string;config:Record<string,unknown>;limits?:Partial<TokenLimits>;notebookId?:string;sourceIds?:string[]}){
  ...
  return <section className="context-capacity" aria-label="上下文容量预估"><b>上下文容量预估</b><p>{result?.basis||error||'正在估算…'}</p>{result?<>{!Object.values(result.strategies||{all:result.strategy}).includes('balanced')?<p>当前使用保守策略。下表是未启用均衡方案的预估容量，质量验证通过前不会自动启用。</p>:null}<small className="context-scroll-hint">左右滑动查看完整容量对照</small><div className="context-capacity-table" tabIndex={0} role="region" aria-label="功能容量对照表，可横向滚动"><table><thead><tr><th>功能</th><th>实际策略</th><th>预计选材</th><th>原文利用路径</th><th>总预算上限</th></tr></thead><tbody>{result.plans.map(plan=>{const saved=result.saved_plans.find(item=>item.kind===plan.kind),delta=saved?plan.estimated_segments-saved.estimated_segments:0;return <tr key={plan.kind}><th>{names[plan.kind]}</th><td>{(result.strategies?.[plan.kind]||result.strategy)==='balanced'?'均衡':'保守'}<small>{result.strategy_reasons?.[plan.kind]}</small></td><td>约 {plan.estimated_segments} 段{delta!==0?<small>（{delta>0?'+':''}{delta}）</small>:null}</td><td>{plan.path==='prepared'?'分区预读后综合':plan.path==='direct'?'完整原文直读':plan.path==='structured'?'按来源与章节选材':plan.path==='query'?'问题相关原文':plan.preparation_batches>0?'分批选材后综合':'直接综合'}{plan.preparation_batches>0?<small>预读 {plan.preparation_batches} 批 · 最多 {plan.note_capacity??'—'} 条笔记</small>:null}{plan.kind==='podcast'?<small>先完成短稿，再逐章深化 · 最多 {plan.podcast_chapters??'—'} 个核心章节 · 短稿输出上限 {plan.core_output_tokens?.toLocaleString()||'—'} tokens · 预读总额 {plan.preparation_token_limit?.toLocaleString()||'—'} tokens</small>:null}{plan.kind==='summary'?<small>摘要容量约 {plan.output_items} 点{plan.overview_items?` · 总览最多 ${plan.overview_items} 点`:''}</small>:null}</td><td>{plan.total_token_limit.toLocaleString()} tokens<small>{plan.limiting_factor}</small></td></tr>})}</tbody></table></div><small>{result.assumptions} 总预算包含输入、输出、推理及重试；安全余量 20%；任务预算随有效窗口和资料量调整，以各功能显示值为准。片段数不是输出段落数。</small></>:null}</section>;
}
...
export function SettingsDrawer({status,providers,roles,imagePolicy,notebookId,sourceIds,onClose,onSave,onCreate,onInspect,onSaveRole,onSaveImagePolicy}:SettingsProps){
  ...
  return <Overlay className={`settings ${isForm?'settings-form-view':''}`} label="Provider 配置" onClose={()=>requestLeave('close')} closeOnBackdrop={false}>
    ...
    {view.mode==='list'?<>
      ...
      <div className="provider-role-grid">{(['main','vlm','audio'] as const).map(role=>{const state=roles.find(item=>item.role===role);const selected=providers.find(item=>item.id===state?.selected_provider_id);return <section className={`provider-role-card ${state?.enabled?'enabled':'disabled'}`} key={role}><div><span>{role.toUpperCase()}</span><h3>{role==='main'?'核心推理':role==='vlm'?'图片理解':'语音生成与验收'}</h3><p>{selected?selected.name:'尚未选择 Provider'}</p></div>{role==='main'?<b className="role-required">必需</b>:<label className="role-switch"><input type="checkbox" checked={Boolean(state?.enabled)} disabled={Boolean(busy)||!selected} onChange={event=>{setBusy(`role-${role}`);void onSaveRole(role,{enabled:event.target.checked,validation_mode:'catalog'}).finally(()=>setBusy(''))}}/><span>{state?.enabled?'已启用':'已暂停'}</span></label>}<button onClick={()=>setView({mode:'role',role})}>管理 {role.toUpperCase()}</button></section>})}</div>
