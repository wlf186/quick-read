...
import type {Provider,ProviderDraft,ProviderInspection,ProviderKind,ProviderModel,ProviderRole} from './api';
...
function ProviderStatus({provider,status}:{provider:Provider;status:any}){
  ...
  return <div className="provider"><i className={!active?'inactive':online?'ok':''}/><span><b>{provider.role.toUpperCase()} · {provider.name}</b><small>{label} · {provider.kind} · {provider.model||'未选择模型'}{provider.config?.compute_device?` · ${String(provider.config.compute_device).toUpperCase()}`:''}{provider.has_api_key?' · KEY SAVED':''}</small></span></div>;
}
