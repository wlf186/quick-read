...
function newDraft(role:ConfigurableProviderRole='main'):ProviderDraft{const kind=KINDS_BY_ROLE[role][0].value;return{name:'',role,kind,base_url:'',model:'',api_key:'',config:role==='audio'?{auto_select:true,asr_auto_select:true,asr_allow_device_fallback:true}:{}}}
...
export function SettingsDrawer({status,providers,roles,imagePolicy,onClose,onSave,onCreate,onInspect,onSaveRole,onSaveImagePolicy}:SettingsProps){
  ...
  function updateRole(role:ConfigurableProviderRole){const kind=KINDS_BY_ROLE[role][0].value;mutate(current=>({...current,role,kind,model:'',config:kind==='sandevistan_audio'?{auto_select:true,asr_auto_select:true,asr_allow_device_fallback:true}:{}}),true,true)}
  ...
  return <Overlay className={`settings ${isForm?'settings-form-view':''}`} label="Provider 配置" onClose={()=>requestLeave('close')} closeOnBackdrop={false}>
    ...
    {view.mode==='list'?<>
      ...
      <div className="provider-form">
        <div className="provider-form-section"><h3>基本信息</h3><label>名称<input value={draft.name} onChange={event=>mutate(current=>({...current,name:event.target.value}),false)}/></label><div className="provider-form-grid"><label>角色<select value={draft.role} disabled><option value="main">MAIN</option><option value="vlm">VLM</option><option value="audio">AUDIO</option>{draft.role==='tts_only'?<option value="tts_only">TTS ONLY</option>:null}</select></label><label>类型<select value={draft.kind} disabled={view.mode==='edit'} onChange={event=>mutate(current=>({...current,kind:event.target.value as ProviderKind,model:'',config:event.target.value==='sandevistan_audio'?{auto_select:true,asr_auto_select:true,asr_allow_device_fallback:true}:{}}),true,true)}>{draft.role==='tts_only'?<option value="openai_tts">OPENAI TTS · LEGACY</option>:KINDS_BY_ROLE[draft.role as ConfigurableProviderRole].map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div></div>
        ...
        <div className="provider-form-section">
          ...
          {draft.kind==='sandevistan_audio'?<>
            <h4>TTS 合成</h4>
            <label className="provider-check"><input type="checkbox" checked={Boolean(draft.config.auto_select)} onChange={event=>mutate(current=>({...current,config:{...current.config,auto_select:event.target.checked}}))}/> 自动选择已安装的最高质量模型</label>
