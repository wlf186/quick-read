...
export type ProviderConfig=Record<string,any>&{temperature?:number;context_window_tokens?:number;max_output_tokens?:number;study_generation_tier?:'auto'|'lite'|'full';auto_select?:boolean;compute_device?:string;allow_device_fallback?:boolean;asr_auto_select?:boolean;asr_model?:string;asr_compute_device?:string;asr_allow_device_fallback?:boolean;host_a_voice_mode?:HostVoiceMode;host_b_voice_mode?:HostVoiceMode;host_a_voiceprint_person_id?:string;host_b_voiceprint_person_id?:string;host_a_voiceprint_sample_id?:string;host_b_voiceprint_sample_id?:string};
export type Provider={id:string;name:string;role:ProviderRole;kind:ProviderKind;base_url:string;model:string;active:number;selected?:number;has_api_key:boolean;capabilities:Record<string,any>&{study_generation?:{tier:'lite'|'full';source:string;reason:string;parameter_count?:number|null;supports_difficulties:string[]}};config:ProviderConfig};
export type ProviderRoleState={role:ConfigurableProviderRole;enabled:boolean;required:boolean;selected_provider_id?:string};
export type ImageProcessor='vlm'|'main'|'ocr';
export type ImageProcessingPolicy={mode:'process'|'off';processors:ImageProcessor[]};
export type WorkspaceState={versions:Record<string,string>;active_jobs:Job[];failed_jobs:Job[];has_active_jobs:boolean};
export type ProviderModel={id:string;name:string;installed?:boolean;voice_modes?:string[];devices?:Array<{id:string;available:boolean;precision?:string;reason?:string}>;controls?:Record<string,any>;details?:Record<string,any>;token_limits?:Partial<TokenLimits>};
export type VoiceprintPersonOption={id:string;name:string;note?:string|null;eligible_sample_count:number;latest_sample?:{id:string;language:string;duration?:number|null;created_at?:string|null}};
export type VoiceprintLibrary={status:'ready'|'unsupported'|'unavailable';people:VoiceprintPersonOption[];message?:string|null};
export type ProviderInspection={status:'passed'|'warning'|'failed';connection_ok:boolean;activation_eligible:boolean;latency_ms:number;catalog_supported:boolean;models:ProviderModel[];capabilities:Record<string,any>;recommended?:{model?:string;compute_device?:string}|null;voiceprint_library?:VoiceprintLibrary;resolved_audio_config?:ProviderConfig;warning?:string|null;error?:{code:string;stage:string;message:string;hint:string;upstream_status?:number|null}|null};
...
export const getJobs=async(id?:string)=>(await getJobsPage({notebook_id:id,page_size:100,view:'summary'})).items;
export const getWorkspaceState=(id:string,signal?:AbortSignal)=>api<WorkspaceState>(`/workspace-state?notebook_id=${encodeURIComponent(id)}`,{signal});
...
export const updateImageProcessingPolicy=(body:ImageProcessingPolicy)=>api<ImageProcessingPolicy>('/settings/image-processing',{method:'PUT',headers:jsonHeaders,body:JSON.stringify(body)});
